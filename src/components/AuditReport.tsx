        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4">
            <div className="text-right">
              ...
              <p className={`text-4xl font-bold ${getScoreColor(result.compliance_score)}`}>
                {result.compliance_score}<span className="text-xl text-muted-foreground">/100</span>
              </p>
            </div>
          </div>
        </div>
            ...
            <div key={i} className="p-4 rounded-lg bg-background/50 border border-border space-y-3">
              <div className="flex items-start justify-between gap-4">
                <div className="space-y-1">
                  <p className="font-mono text-xs text-accent bg-accent/10 inline-block px-2 py-0.5 rounded">
                    {v.element}
                  </p>
                  <p className="text-sm font-medium text-foreground">{v.violation}</p>
                </div>
              </div>
              <div className="flex gap-2 items-start text-sm text-muted-foreground bg-card/50 p-3 rounded-md border border-border/50">
                ...
                <p><span className="font-medium text-foreground">Fix:</span> {v.fix}</p>
              </div>
            </div>
          ))}
        </div>
